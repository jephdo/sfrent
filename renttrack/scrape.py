...
def scrape_craigslist(max_price=10000, min_price=1000, limit=None):
    cl = CraigslistHousing(site='sfbay', area='sfc', category='apa',
                           filters={'max_price': max_price, 'min_price': min_price, 
                                    'private_room': True, 'posted_today': True})
    listings = []
    for result in cl.get_results(sort_by='newest', geotagged=True, limit=limit):
        bedrooms = int(result['bedrooms']) if result['bedrooms'] is not None else 0
